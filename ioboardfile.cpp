...
void IOBoardFile::loadFile(QString path)
{
    m_loaded=false;
#ifdef ANDROID
    path=QString(getenv("EXTERNAL_STORAGE"))+"/staTIc/"+path;
#else
    path=QString(getenv("HOME"))+"/"+path;
#endif
    QFile file(path);
...
{
#ifdef ANDROID
    QString filename=QString(getenv("EXTERNAL_STORAGE"))+"/staTIc/";
    filename=filename+"board_conf_"+ QDateTime::currentDateTime().toString("dMyy_Hms")+".data";
#else
    QString filename="board_conf_"+ QDateTime::currentDateTime().toString("dMyy_Hms")+".data";
#endif
    QFile file(filename);
